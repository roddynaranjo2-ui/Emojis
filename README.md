...
| **Content** | 144 curated emojis · 12 families · 20 evolution chains · 71 cross recipes · **7 worlds × 20 = 140 levels** (auto‑tuned) |
...
Map ⇄ 📖 Emoji‑dex (silhouettes, rarity, echoes) · 🧪 Laboratory (mix A+B) · 🛒 Shop · ⚙️ Options · 🎁 Daily gift (7‑day streak)
```
...
packages/
  ...
  content/   … levels.ts (140‑level generator) + tuning.ts (auto‑tuned move budgets) + validate.ts (CI gate)
  ...
  app/       main.ts (router + level flow) · state.ts (economy & save v2) · screens/{splash,map,game,popups,dex,lab,shop,settings}.ts
tools/
...
| `npm test` | 42 specs: shapes, specials, **all synergies**, blockers, dust, cages, holes, boosters, tap‑to‑fire, +moves, cascades, determinism, mercy |
...
| **Current balance** | `sim 12`: **140/140 levels in band**, world averages 55–62 % (bot) |
